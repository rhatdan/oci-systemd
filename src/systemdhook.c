	...
	if ((dir = opendir (path)) != NULL) {
		...
		while ((ent = readdir (dir)) != NULL) {
			...
			if (setfilecon (full_path, label) < 0) {
				pr_perror("Failed to set context %s on %s", path, label);
			}
		}
		closedir (dir);
	} else {
 ...
 */
static int mount_cgroup(const char *rootfs, const char *options)
{
		...
		unsigned config_mounts_len)
{
	...
	if (!contains_mount(config_mounts, config_mounts_len, CGROUP_ROOT)) {
		rc = mount_cgroup(rootfs, options);
	}
