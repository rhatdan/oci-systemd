 ...
 */
static int move_mount_to_tmp(const char *rootfs, const char *tmp_dir, const char *mount_dir)
{
	int rc;
...
static int move_mounts(const char *rootfs,
	...
	if (!contains_mount(config_mounts, config_mounts_len, path)) {
		...
		if (strcmp("/run", path) == 0) {
			if (move_mount_to_tmp(rootfs, tmp_dir, "/run/secrets") < 0) {
		...
		for (unsigned i = 0; i < config_mounts_len; i++) {
			...
			if (!strncmp(path, config_mounts[i], strlen(path))) {
				if (move_mount_to_tmp(rootfs, tmp_dir, config_mounts[i]) < 0) {
		...
		unsigned config_mounts_len)
{
	...
	move_mounts(rootfs, "/run", config_mounts, config_mounts_len, options);
	...
	if (!contains_mount(config_mounts, config_mounts_len, "/var/log/journal")) {
		...
		move_mounts(rootfs, "/var/log", config_mounts, config_mounts_len, options);
	...
	if (!contains_mount(config_mounts, config_mounts_len, "/tmp")) {
		...
		move_mounts(rootfs, "/tmp", config_mounts, config_mounts_len, options);
	}
